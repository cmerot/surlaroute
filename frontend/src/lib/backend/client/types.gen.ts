// This file is auto-generated by @hey-api/openapi-ts

export type ActorAssoc_Explore = {
	data?: {
		[key: string]: unknown;
	} | null;
	type?: 'actor';
	actor: Org_Explore | Person_Explore;
};

export type ActorLiteAssoc_Explore = {
	data?: {
		[key: string]: unknown;
	} | null;
	type?: 'actor';
	actor: OrgLite_Explore | PersonLite_Explore;
};

export type AddressPublic = {
	q: string;
	geom_point?: Point | null;
};

export type BaseModel = {
	[key: string]: unknown;
};

export type Body_login_access_token = {
	grant_type?: string | null;
	username: string;
	password: string;
	scope?: string;
	client_id?: string | null;
	client_secret?: string | null;
};

export type ContactPublic = {
	email_address?: string | null;
	phone_number?: string | null;
	address?: AddressPublic | null;
	website?: string | null;
};

/**
 * Used for the /tours/{id} endpoint
 */
export type EventLitePublic = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	description?: string | null;
	start_dt: string;
	end_dt?: string | null;
	actor_assocs?: Array<ActorAssoc_Explore>;
};

/**
 * Feature Model
 */
export type Feature = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'Feature';
	geometry:
		| (
				| Point
				| MultiPoint
				| LineString
				| MultiLineString
				| Polygon
				| MultiPolygon
				| GeometryCollection
		  )
		| null;
	properties:
		| {
				[key: string]: unknown;
		  }
		| BaseModel
		| null;
	id?: number | string | null;
};

/**
 * FeatureCollection Model
 */
export type FeatureCollection = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'FeatureCollection';
	features: Array<Feature>;
};

/**
 * GeometryCollection Model
 */
export type GeometryCollection = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'GeometryCollection';
	geometries: Array<
		Point | MultiPoint | LineString | MultiLineString | Polygon | MultiPolygon | GeometryCollection
	>;
};

export type HTTPValidationError = {
	detail?: Array<ValidationError>;
};

/**
 * LineString Model
 */
export type LineString = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'LineString';
	coordinates: Array<Position2D | Position3D>;
};

export type Message = {
	message: string;
};

/**
 * MultiLineString Model
 */
export type MultiLineString = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'MultiLineString';
	coordinates: Array<Array<Position2D | Position3D>>;
};

/**
 * MultiPoint Model
 */
export type MultiPoint = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'MultiPoint';
	coordinates: Array<Position2D | Position3D>;
};

/**
 * MultiPolygon Model
 */
export type MultiPolygon = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'MultiPolygon';
	coordinates: Array<Array<Array<Position2D | Position3D>>>;
};

export type NewPassword = {
	token: string;
	new_password: string;
};

export type Org_Explore = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Org';
	description?: string | null;
	activities: Array<TreePublic>;
	pyd_org?: string;
	membership_assocs: Array<OrgLiteAssoc_Explore>;
	member_assocs: Array<ActorLiteAssoc_Explore>;
};

export type OrgAssoc_Explore = {
	data?: {
		[key: string]: unknown;
	} | null;
	type?: 'org';
	org: Org_Explore;
};

export type OrgFull_Explore = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Org';
	description?: string | null;
	activities: Array<TreePublic>;
	pyd_org?: string;
	membership_assocs: Array<OrgAssoc_Explore>;
	member_assocs: Array<ActorAssoc_Explore>;
	tour_assocs?: Array<TourActorAssoc>;
};

export type OrgLite_Explore = {
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Org';
	description?: string | null;
	activities: Array<TreePublic>;
};

export type OrgLiteAssoc_Explore = {
	data?: {
		[key: string]: unknown;
	} | null;
	type?: 'org';
	org: OrgLite_Explore;
};

export type OwnerPersonPublic = {
	id: string;
	name: string;
};

export type OwnerPublic = {
	person?: OwnerPersonPublic | null;
};

export type PagedResponse_TourPublic_ = {
	total: number;
	limit: number;
	offset: number;
	results: Array<TourPublic>;
};

export type PagedResponse_TreePublic_ = {
	total: number;
	limit: number;
	offset: number;
	results: Array<TreePublic>;
};

export type PagedResponse_Union_Org_Explore__Person_Explore__ = {
	total: number;
	limit: number;
	offset: number;
	results: Array<Org_Explore | Person_Explore>;
};

export type PagedResponse_Union_PersonFull_Explore__OrgFull_Explore__ = {
	total: number;
	limit: number;
	offset: number;
	results: Array<PersonFull_Explore | OrgFull_Explore>;
};

export type PagedResponse_UserPublic_ = {
	total: number;
	limit: number;
	offset: number;
	results: Array<UserPublic>;
};

export type Person_Explore = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Person';
	role?: string | null;
	pyd_person?: string;
	membership_assocs: Array<OrgLiteAssoc_Explore>;
};

export type PersonFull_Explore = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Person';
	role?: string | null;
	pyd_person?: string;
	membership_assocs: Array<OrgAssoc_Explore>;
};

export type PersonLite_Explore = {
	id: string;
	name: string;
	contact?: ContactPublic | null;
	type: 'Person';
	role?: string | null;
};

/**
 * Point Model
 */
export type Point = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'Point';
	coordinates: Position2D | Position3D;
};

/**
 * Polygon Model
 */
export type Polygon = {
	bbox?: [number, number, number, number] | [number, number, number, number, number, number] | null;
	type: 'Polygon';
	coordinates: Array<Array<Position2D | Position3D>>;
};

export type Position2D = [number, number];

export type Position3D = [number, number, number];

export type Token = {
	access_token: string;
	token_type?: string;
};

export type TourActorAssoc = {
	tour: TourLitePublic;
};

/**
 * Used for the /tours endpoint
 */
export type TourLitePublic = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	description?: string | null;
	year?: number | null;
	disciplines: Array<TreePublic>;
	mobilities: Array<TreePublic>;
};

/**
 * Used for the /tours/{id} endpoint
 */
export type TourPublic = {
	owner_id?: string | null;
	group_owner_id?: string | null;
	other_read: boolean;
	member_read: boolean;
	group_read: boolean;
	owner?: OwnerPublic | null;
	id: string;
	name: string;
	description?: string | null;
	year?: number | null;
	events: Array<EventLitePublic>;
	disciplines: Array<TreePublic>;
	mobilities: Array<TreePublic>;
	actor_assocs?: Array<ActorAssoc_Explore>;
};

export type TreePublic = {
	name: string;
	path: string;
};

export type UserCreate = {
	email: string;
	is_active?: boolean;
	is_superuser?: boolean;
	is_member?: boolean;
	password: string;
};

export type UserPerson = {
	id: string;
	name: string;
	membership_assocs: Array<UserPersonOrgAssoc>;
};

export type UserPersonOrg = {
	id: string;
	name: string;
};

export type UserPersonOrgAssoc = {
	data: {
		[key: string]: unknown;
	};
	org: UserPersonOrg;
};

export type UserPublic = {
	email: string;
	is_active: boolean;
	is_superuser: boolean;
	is_member: boolean;
	id: string;
	person?: UserPerson | null;
};

export type UserRegister = {
	email: string;
	password: string;
};

export type UserUpdate = {
	email?: string | null;
	is_active?: boolean;
	is_superuser?: boolean;
	is_member?: boolean;
	password?: string | null;
};

export type ValidationError = {
	loc: Array<string | number>;
	msg: string;
	type: string;
};

export type ExploreGetActorsData = {
	query?: {
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type ExploreGetActorsResponse = PagedResponse_Union_Org_Explore__Person_Explore__;

export type ExploreGetActorsError = HTTPValidationError;

export type LoginAccessTokenData = {
	body: Body_login_access_token;
};

export type LoginAccessTokenResponse = Token;

export type LoginAccessTokenError = HTTPValidationError;

export type LoginTestTokenResponse = UserPublic;

export type LoginTestTokenError = unknown;

export type LoginRecoverPasswordData = {
	path: {
		email: string;
	};
};

export type LoginRecoverPasswordResponse = Message;

export type LoginRecoverPasswordError = HTTPValidationError;

export type LoginResetPasswordData = {
	body: NewPassword;
};

export type LoginResetPasswordResponse = Message;

export type LoginResetPasswordError = HTTPValidationError;

export type LoginRecoverPasswordHtmlContentData = {
	path: {
		email: string;
	};
};

export type LoginRecoverPasswordHtmlContentResponse = string;

export type LoginRecoverPasswordHtmlContentError = HTTPValidationError;

export type UsersReadData = {
	query?: {
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type UsersReadResponse = PagedResponse_UserPublic_;

export type UsersReadError = HTTPValidationError;

export type UsersCreateData = {
	body: UserCreate;
};

export type UsersCreateResponse = UserPublic;

export type UsersCreateError = HTTPValidationError;

export type UsersReadUserMeResponse = UserPublic;

export type UsersReadUserMeError = unknown;

export type UsersRegisterData = {
	body: UserRegister;
};

export type UsersRegisterResponse = UserPublic;

export type UsersRegisterError = HTTPValidationError;

export type UsersReadByIdData = {
	path: {
		user_id: string;
	};
};

export type UsersReadByIdResponse = UserPublic;

export type UsersReadByIdError = HTTPValidationError;

export type UsersUpdateData = {
	body: UserUpdate;
	path: {
		user_id: string;
	};
};

export type UsersUpdateResponse = UserPublic;

export type UsersUpdateError = HTTPValidationError;

export type UsersDeleteData = {
	path: {
		user_id: string;
	};
};

export type UsersDeleteResponse = Message;

export type UsersDeleteError = HTTPValidationError;

export type UtilsTestEmailData = {
	query: {
		email_to: string;
	};
};

export type UtilsTestEmailResponse = Message;

export type UtilsTestEmailError = HTTPValidationError;

export type UtilsHealthCheckResponse = boolean;

export type UtilsHealthCheckError = unknown;

export type ActivitiesReadActivitiesByPathData = {
	path: {
		path: string;
	};
	query?: {
		descendant?: boolean;
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type ActivitiesReadActivitiesByPathResponse = PagedResponse_TreePublic_ | TreePublic;

export type ActivitiesReadActivitiesByPathError = HTTPValidationError;

export type ActivitiesReadActivitiesData = {
	query?: {
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type ActivitiesReadActivitiesResponse = PagedResponse_TreePublic_;

export type ActivitiesReadActivitiesError = HTTPValidationError;

export type DirectoryGetActorsData = {
	query?: {
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type DirectoryGetActorsResponse = PagedResponse_Union_PersonFull_Explore__OrgFull_Explore__;

export type DirectoryGetActorsError = HTTPValidationError;

export type DirectoryGetOrgDetailsData = {
	path: {
		id: string;
	};
};

export type DirectoryGetOrgDetailsResponse = OrgFull_Explore;

export type DirectoryGetOrgDetailsError = HTTPValidationError;

export type DirectoryGetPersonDetailsData = {
	path: {
		id: string;
	};
};

export type DirectoryGetPersonDetailsResponse = PersonFull_Explore;

export type DirectoryGetPersonDetailsError = HTTPValidationError;

export type ToursGetTourDetailsGeojsonData = {
	path: {
		id: string;
	};
};

export type ToursGetTourDetailsGeojsonResponse = FeatureCollection;

export type ToursGetTourDetailsGeojsonError = HTTPValidationError;

export type ToursGetTourDetailsData = {
	path: {
		id: string;
	};
};

export type ToursGetTourDetailsResponse = TourPublic;

export type ToursGetTourDetailsError = HTTPValidationError;

export type ToursGetToursData = {
	query?: {
		limit?: number;
		offset?: number;
		q?: string | null;
	};
};

export type ToursGetToursResponse = PagedResponse_TourPublic_;

export type ToursGetToursError = HTTPValidationError;
