<script lang="ts">
	...
	import { Account, Directory, Home, Logo, Membership, Tour, Users } from "$lib/components/icons";
	import NavMain from "$lib/components/layout/sidebar/nav-main.svelte";
	import * as Sidebar from "$lib/components/ui/sidebar";
	import { auth } from "$lib/stores/auth";
	import { ChartLine, Map as LucidMap, Palette, type Icon as IconType } from "lucide-svelte";
	...
	$effect(() => {
		...
		for (const k of Object.keys(staticNavData)) {
			for (const item of staticNavData[k as keyof typeof staticNavData]) {
				const isActive =
					item.url === "/" ? page.url.pathname === "/" : page.url.pathname.startsWith(item.url);
...
<Sidebar.Root bind:ref class="border-r-0" {...restProps}>
	...
	<Sidebar.Content>
		...
		{/if}
		<NavSecondary items={staticNavData.navFavorites} title="Favoris" />
		{#if user?.is_superuser}
			<NavSecondary items={staticNavData.navAdmin} title="Admin" />
		{/if}
	</Sidebar.Content>
</Sidebar.Root>
